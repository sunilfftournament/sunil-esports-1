...
// 4. FIREBASE CONFIG: Ye sabse jaruri hai. 
// Agar tum GitHub/Vercel pe daal rahe ho, toh is section ko uncomment karke apni keys bharo.
// Filhal ye "Safe Mode" mein chalega agar keys nahi hain.
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Check if provided by environment (e.g. preview)
    try {
    ...
    const initFirebase = async () => {
        try {
            if (!configToUse || !configToUse.apiKey) throw new Error("No API Key");
            ...
            try {
                ...
                console.error("Auth Error (Offline Mode):", authErr.code);
  ...
  useEffect(() => {
      ...
      const safeSub = (ref, cb) => onSnapshot(ref, cb, (e) => {
          console.log("Sync Error:", e.code);
          if (e.code === 'permission-denied' || e.code === 'unavailable') setIsOfflineMode(true);
      });
      ...
      const unsubUser = safeSub(doc(db, 'artifacts', appId, 'public', 'data', 'user_stats', user.uid), (s) => { 
          if(s.exists()) { 
              const b = s.data().balance||0; 
              setBalance(b); localStorage.setItem('myBalance', b); localStorage.setItem('balHash', hashBalance(b)); 
          } 
      });
  ...
  const updateBalance = async (newBal, uid = user?.uid) => {
      ...
      localStorage.setItem('balHash', hashBalance(newBal));
  ...
  return (
    <div className="font-sans min-h-screen text-gray-100 overflow-x-hidden bg-slate-900 pb-20" style={{backgroundColor: '#0f172a', color: 'white', minHeight: '100vh'}}>
      ...
      <nav className="fixed w-full z-50 border-b border-cyan-500/20 bg-black/80 backdrop-blur-md">
        <div className="container mx-auto px-4 py-3 flex justify-between items-center">
            <div className="flex items-center gap-3 cursor-pointer select-none" onClick={handleLogoTap}>
                ...
                <div className="flex flex-col"><span className="text-xl font-bold tracking-widest text-white leading-none">SUNIL LIVE <span className="text-yellow-400">ESPORTS</span></span><span className="text-[10px] text-gray-400">UID: {user?.uid.slice(0,6)}</span></div>
            </div>
      ...
      {view === 'admin' && (
          ...
          <div className="flex gap-2 mb-6">{['matches', 'users'].map(tab => <button key={tab} onClick={() => setAdminTab(tab)} className={`px-4 py-2 rounded font-bold text-xs uppercase ${adminTab===tab?'bg-red-600':'bg-slate-800'}`}>{tab}</button>)}</div>
      ...
      {view === 'wallet' && (
          <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-md flex items-center justify-center p-4">
              <div className="bg-slate-900 border border-yellow-500/50 w-full max-w-sm rounded-2xl p-4"><div className="flex justify-between mb-4"><h3 className="font-bold text-white flex gap-2"><Wallet className="text-yellow-400"/> VAULT</h3><button onClick={()=>setView('home')}><X/></button></div>
              <h2 className="text-4xl font-black text-center mb-6">₹{balance}</h2>
